function batchOpt = generateClickTrainSSAStimparaECoG(params,datapath)
...
for i = 1 : length(sd)
stimType{i} =  strrep(stimType{i},'SD1',num2str(sd(i)));
end
elseif contains(datapath,'ICIThr')
stimType = stimStr(SSAPairs);
for i = 1 : length(ICIThr)
stimType{i} =  strrep(stimType{i},'SD1',num2str(ICIThr(i)));
end
else
stimType = stimStr(SSAPairs);
