function TrialParas=PEOddBaseLabelStr(TrialParas,varargin)
...
FreqIndex=find([TrialParas.FreqDiff]'>1&[TrialParas.IntDiff]'==1);
for ind=1:length(FreqIndex)
TrialParas(FreqIndex(ind)).CueType='freq';
end


IntIndex=find([TrialParas.FreqDiff]'==1&[TrialParas.IntDiff]'>1);
for ind=1:length(IntIndex)
TrialParas(IntIndex(ind)).CueType='int';
end
