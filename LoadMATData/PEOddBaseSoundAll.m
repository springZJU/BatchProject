...
if ~isfield(dat.epocs,'push')
    ...
    %%
    sweeBuffer = dat.epocs.swee.data;
    numBuffer = dat.epocs.num0.data;
    swee = unique(sweeBuffer);
    for s = 1:length(swee)
        idx = sweeBuffer==swee(s);
        if ~ismember(1,numBuffer(idx))
            dat.epocs.num0.data(find(idx)) = 0;
        end
    end
...
for k=1:length(devsti)%
    ...
    j=find(Int==devsti(k,13));% p：int dev ratio
