...
for dev_diff=Diff
    ...
    for ch=Chs
        count=count+1;
        Res{ch-Chs(1)+1,dev_diff}=rmfield(Spect,'Single');
        if ToPlot==1
...
%         end
        ...
        for dev_diff=Diff
            if strcmp(Cscale,'auto_equalCLim')
                set(Ax{ch,dev_diff},'CLim',[-CLimChMaxAll CLimChMaxAll]);
            elseif strcmp(Cscale,'auto_semiequalCLim')
                set(Ax{ch,dev_diff},'CLim',[0 CLimChMaxAll]);
            end
